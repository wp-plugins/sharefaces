 ...
 * @revision   jQueryRevision: 4652 jQuery
 * @changed    jQueryDate: 2010-12-01 11:35:11 +0100 (Wed, 01 Dec 2010) jQuery
 * @access     public
 */
(function(jQuery) {
	var effects={};
	...
	jQuery.fn.theatre=function() {
		...
		(this.length ? this : jQuery(document)).each(function() {
				methods.initAll.apply(jQuery(this), currArguments);
			});
		return this;
	}
	...
	methods.loadEffect=function(name) { 
		if (!urlBase) { // Find the locations
			jQuery('script[src*="jquery.theatre-"], link[href*="theatre.css"]').first().each(function() {
					urlBase=(jQuery(this).attr('href') || jQuery(this).attr('src')).replace(/\/[^\/]*(#.*)?jQuery/, '');
				});
		}
	...
	methods.initAll=function(method) {
		...
		if (method == 'effect') {
			...
			if (typeof arguments[2] == 'function') {
				...
				jQuery.error("Elixon Theatre cannot register effect object unless it is a Function.");
			}
		}
		...
		case 'stop':		
		    ...
		    break;
		default: // Unsupported
			jQuery.error('Elixon Theatre method "'+method+'" does not exist on jQuery.theatre!');
		}
	}
	...
	methods.init=function(options) {
		...
		// Init effect object
		var actors=jQuery(settings.selector, this);
		...
		if (!effect) jQuery.error('Elixon Theatre does not support effect "'+settings.effect+'"!');
		...
		// Actors
		actors.each(function(){if (!jQuery(this).data('theatre')) jQuery(this).data('theatre', {width: jQuery(this).width(), height: jQuery(this).height()});});		
		if (settings.itemWidth || settings.itemHeight) {
			var thisObj=this;
			actors.each(function() {
					if (settings.itemWidth) {
						jQuery(this).css('width', settings.itemWidth == 'max' ? (thisObj.width() - jQuery(this).outerWidth() + jQuery(this).width())+'px' : settings.itemWidth);
					}
					if (settings.itemHeight) {
						jQuery(this).css('height', settings.itemHeight == 'max' ? (thisObj.height() - jQuery(this).outerHeight() + jQuery(this).height())+'px' : settings.itemHeight);
					}
				});
		}
	...
	methods.appendControls=function() {
		...
		if (settings.controls == 'horizontal' || settings.controls == 'vertical') {
			...
			jQuery('.theatre-next', this).click(function() {thisObj.theatre('next');});
			jQuery('.theatre-prev', this).click(function() {thisObj.theatre('prev');});
			//jQuery('.theatre-play', this).click(function() {thisObj.theatre('play');});
			//jQuery('.theatre-stop', this).click(function() {thisObj.theatre('stop');});
			this.mouseenter(function() {jQuery('.theatre-control', thisObj).fadeIn();});
			this.mouseleave(function() {jQuery('.theatre-control', thisObj).fadeOut();});
			jQuery('.theatre-control', this).fadeOut(0);
		}
	...
	methods.destroy=function() {
		...
		if (theatre) { 
			...
			theatre.actors.each(function(){ // Restore original sizes
					var theatre=jQuery(this).data('theatre');
					jQuery(this).width(theatre.width);
					jQuery(this).height(theatre.height);
				})
		}
		jQuery('.theatre-control', this).remove();
	}
	...
	effects['horizontal']=function(stage, actors, settings) {
		...
		this.next=function() {
			// jQuery(settings.selector, stage).last().stop(true, true); // to prevent visual contra-direction transition
			var curr=jQuery(settings.selector, stage).first();
			var offset=this.align(-curr[x.size]());
			curr.appendTo(stage);
		}
		this.prev=function() {
			var curr=jQuery(settings.selector, stage).last().prependTo(stage);
		...
		this.align=function(offset, speed) {
			var sacked=false;
			jQuery(settings.selector, stage).each(function() {
					var callBack=null
					if (offset < 0) {
						sacked=jQuery(this);
					...
					jQuery(this).stop(true, true).animate(props, isNaN(speed) ? settings.speed : speed, callBack);
					offset+=jQuery(this)[x.size]();
				});
			return offset;
		}
	}
	...
	effects['3d']=function(stage, actors, settings) {
		...
		this.animate=function() {
			...
			actors.each(function(pos) {
					...
					var dim=thisObj.calcDim(jQuery(this), maxWidth, maxHeight, pivot.size);
					...
					jQuery(this).css({'z-index': Math.round(pivot.size * 1000)}).animate({opacity: pivot.size, left: left, top: top, width: dim[0], height: dim[1]}, settings.speed);
				});
		}
